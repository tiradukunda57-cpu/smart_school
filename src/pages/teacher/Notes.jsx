...
import {
  FiPlus, FiTrash2, FiEdit, FiFileText,
  FiDownload, FiSearch, FiEye
} from 'react-icons/fi'
...
const emptyForm = { title: '', content: '', subject: '', category: 'Lecture' }
const CATEGORIES = ['Lecture', 'Summary', 'Reference', 'Exercise', 'Announcement']
...
export default function Notes() {
  ...
  const openEdit = (n) => {
    setSelected(n)
    setForm({ title: n.title, content: n.content, subject: n.subject || '', category: n.category || 'Lecture' })
    setErrors({})
    setEditModal(true)
  }
    ...
    <DashboardLayout>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{notes.length} notes shared with students</p>
        </div>
        <Button
          variant="primary"
          icon={<FiPlus size={16} />}
          onClick={() => { setForm(emptyForm); setErrors({}); setCreateModal(true) }}
        >
          New Note
        </Button>
      </div>
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.5rem' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {note.subject && (
                      <p style={{ fontSize: '0.78rem', color: 'var(--secondary)', fontWeight: 600, marginTop: '0.2rem' }}>
                        {note.subject}
                      </p>
                    )}
                  </div>
                ...
                <div style={{
                  ...
                  <button
                    onClick={() => { setSelected(note); setDetailModal(true) }}
                  ...
                  <button onClick={() => openEdit(note)} style={{ ...actionBtn, flex: 1, color: 'var(--secondary)' }}>
                    <FiEdit size={13} />
                  </button>
                  <button onClick={() => { setSelected(note); setDeleteModal(true) }}
      ...
      {/* ── Create Modal ── */}
      <NoteFormModal
        isOpen={createModal} onClose={() => setCreateModal(false)}
        title="New Note" form={form} setForm={setForm}
        errors={errors} saving={saving} onSubmit={handleCreate}
      ...
      <NoteFormModal
        ...
        title="Edit Note" form={form} setForm={setForm}
        errors={errors} saving={saving} onSubmit={handleEdit}
        submitLabel="Save Changes"
      />

      {/* ── Detail Modal ── */}
      <Modal
        isOpen={detailModal} onClose={() => setDetailModal(false)}
        title="Note Details" size="lg"
        footer={
          selected && (
            <Button variant="secondary" icon={<FiDownload size={15} />}
              onClick={() => { handleDownload(selected); setDetailModal(false) }}>
              Download PDF
            </Button>
          )
        }
        ...
        {selected && (
          <div>
            <div style={{
              ...
              <h2 style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--primary)', marginBottom: '0.5rem' }}>
                {selected.title}
              </h2>
      ...
      <Modal
        ...
        title="Delete Note" size="sm"
        footer={
          <>
            <Button variant="ghost" onClick={() => setDeleteModal(false)}>Cancel</Button>
            <Button variant="danger" loading={saving} onClick={handleDelete}>Delete Note</Button>
          </>
        }
      >
        <div style={{ textAlign: 'center', padding: '0.5rem 0' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>
            Delete note <strong>"{selected?.title}"</strong>? Students will lose access to this note.
          </p>
        </div>
      </Modal>
    </DashboardLayout>
...
function NoteFormModal({ isOpen, onClose, title, form, setForm, errors, saving, onSubmit, submitLabel }) {
  const handleChange = e => setForm(p => ({ ...p, [e.target.name]: e.target.value }))
  return (
    <Modal
      isOpen={isOpen} onClose={onClose} title={title} size="lg"
      footer={
        <>
          <Button variant="ghost" onClick={onClose}>Cancel</Button>
          <Button variant="primary" loading={saving} onClick={onSubmit}>{submitLabel}</Button>
        </>
      }
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
        <div className="form-group">
          ...
          <input className="form-input" name="subject" value={form.subject}
            onChange={handleChange} placeholder="e.g. Physics" />
        </div>
        <div className="form-group">
